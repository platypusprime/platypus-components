...
public class PColorButton extends JComponent {
     ...
     * 
     * @param size button size
     * @param steps granularity of fade
     * @param interval total time for fade, in ms
     * @param idleColor button color when not being interacted with
     * @param mouseOverColor button color when pointer is over it
     */
    public PColorButton(Dimension size, int steps, int interval,
            Color idleColor, Color mouseOverColor) {
     ...
     * @param idleColor button color when not being interacted with
     * @param mouseOverColor button color when pointer is over it
     * @param mouseDownColor button color when mouse is pressed on the button
     */
    public PColorButton(Dimension size, int steps, long interval,
            Color idleColor, Color mouseOverColor, Color mouseDownColor) {
    ...
    private void calculateDeltas(int i) {
        if (mouseOverColor != null) {
            delta[0] = (mouseOverColor.getRed() - idleColor.getRed()) * i / fadeSteps;
            delta[1] = (mouseOverColor.getGreen() - idleColor.getGreen()) * i / fadeSteps;
            delta[2] = (mouseOverColor.getBlue() - idleColor.getBlue()) * i / fadeSteps;
        }
    }
     ...
     * 
     * @param idleColor the default button color
     ...
     * 
     * @param command the <code>ActionCommand</code> to be set
     */
